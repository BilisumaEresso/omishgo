...
const AppButton = ({
  ...
  return <Pressable onPress={onPress} disabled={isInteractionDisabled} style={({
    pressed
  }) => getButtonStyles(pressed)} accessibilityRole="button" accessibilityState={{
    disabled: isInteractionDisabled,
    busy: loading
  }}>
      {loading ? <ActivityIndicator color={getTextColor()} size="small" /> : <AppText variant="button" style={{
      color: getTextColor(),
      fontWeight: "600"
    }}>
          {title}
