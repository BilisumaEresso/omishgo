...
// Check if native AIRMap view manager exists in the currently running APK binary
const hasNativeMapModule = !!(
...
// React Error Boundary to catch any native MapView crashes on Android APKs
class MapErrorBoundary extends Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.warn('[OrderProgressMap] MapView error caught in boundary:', error, errorInfo);
  }
...
function MapFallbackCard({ originName, destName, progressFraction, isCancelled, primaryColor, textPrimary, textSecondary, t }) {
  return (
...
export default function OrderProgressMap({ order }) {
  ...
  // If currently running APK binary lacks native react-native-maps C++/Java module, render fallback directly
