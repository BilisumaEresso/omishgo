...
export default function OrderDetailScreen({ route, navigation }) {
  ...
  const handleCall = (phone) => {
    if (!phone) {
      Alert.alert(
        t("orderDetail.noPhoneTitle"),
        t("orderDetail.noPhoneMessage"),
      );
      return;
    }
    Linking.openURL(`tel:${phone}`).catch(() =>
      Alert.alert(t("orderDetail.errorTitle"), t("orderDetail.diallerError")),
    );
  };
  ...
  const doUpdate = async (newStatus) => {
    ...
    try {
      const res = await api.patch(
        API_ENDPOINTS.orders.updateStatus(order._id),
        {
          status: newStatus,
        },
      );
      ...
      Alert.alert(
        ...
        t("orderDetail.orderStatusUpdated", {
      ...
      Alert.alert(
        ...
        err?.response?.data?.message || t("orderDetail.updateError"),
      );
    } finally {
      setUpdating(false);
    }
  ...
  return (
    <View style={[styles.container, { backgroundColor: background }]}>
      {/* Header */}
      <AppHeader
        title={t("orderDetail.screenTitle")}
        showBack={true}
      ...
      >
        ...
        >
          ...
          <InfoRow
            icon="cash"
            label={t("orderDetail.pricePerUnit")}
            value={`${pricePerUnit} ETB`}
          />
          <InfoRow
            icon="pricetag"
            label={t("orderDetail.total")}
            value={`${totalPrice} ETB`}
          />
        ...
        >
          <AppText style={[styles.sectionTitle, { color: textPrimary }]}>
            {role === "farmer"
              ? t("orderDetail.buyerInfo")
              : t("orderDetail.farmerInfo")}
          </AppText>
          {role === "farmer" ? (
            <>
              <InfoRow
                icon="person"
                label={t("orderDetail.name")}
                value={buyerName}
              />
              <PhoneRow
                ...
                placeholder={t("orderDetail.notProvided")}
              />
            </>
          ) : (
            <>
        ...
        <View style={styles.actions}>
          {role === "buyer" &&
            ...
            rawStatus !== "delivered" && (
              <AppButton
                title={t("orderDetail.messageFarmer")}
                variant="primary"
                fullWidth
          ...
          {nextActions.map((action) => (
            <AppButton
              key={action.value}
              title={updating ? t("orderDetail.updating") : action.label}
              ...
              style={styles.actionBtn}
              textStyle={
                action.variant === "danger" ? { color: errorColor } : undefined
              }
