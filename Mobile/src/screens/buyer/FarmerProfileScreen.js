...
export default function FarmerProfileScreen({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [farmerRes, productsRes] = await Promise.all([api.get(API_ENDPOINTS.users.detail(farmerId)), api.get(`${API_ENDPOINTS.products.list}?farmerId=${farmerId}`)]);
  ...
  const renderHeader = () => <View style={[styles.profileHeader, {
    backgroundColor: surface
      ...
      textAlign: "center"
        ...
        {t("farmerProfile.activeListings", {
        count: products.length
      })}
      </AppText>
      <View style={styles.chatButtonContainer}>
        <AppButton 
          title={t("farmerProfile.chatNow") || "Chat Now"} 
  ...
  return <View style={[styles.container, {
    backgroundColor: background
      ...
      <AppHeader title={t("farmerProfile.title")} showBack={true} onBackPress={() => navigation.goBack()} />
      <FlatList data={products} keyExtractor={item => item._id} ListHeaderComponent={renderHeader} contentContainerStyle={styles.listContainer} showsVerticalScrollIndicator={false} renderItem={({
      item
    }) => <ProductCard product={item} theme={theme} isSaved={savedIds.has(item._id)} onToggleSave={toggleSave} onView={product => navigation.navigate("ListingDetail", {
      product
