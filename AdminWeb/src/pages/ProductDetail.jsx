    ...
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link to="/products" className="w-10 h-10 rounded-[10px] bg-white border border-border flex items-center justify-center hover:bg-surface-muted transition-colors">
          ...
          <div>
            <h1 className="text-[28px] font-bold text-ink">Product Detail</h1>
            <span className="text-[13px] font-mono text-primary">{product.customId || product._id}</span>
          </div>
        </div>
        ...
        {product.status === "pending" && (
          <div className="flex gap-3">
            <button 
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-success text-white rounded-[10px] font-semibold text-[14px] hover:bg-[#00a08a] transition-colors shadow-sm"
            >
            ...
            <button 
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-danger text-white rounded-[10px] font-semibold text-[14px] hover:bg-[#e04848] transition-colors shadow-sm"
            >
                ...
                {photos.map((photo, i) => (
                  <button
                    ...
                    className={`w-[72px] h-[72px] rounded-[10px] overflow-hidden border-2 transition-all flex-shrink-0 ${
                      activePhoto === i ? 'border-primary shadow-md' : 'border-transparent opacity-60 hover:opacity-100'
                    }`}
                  >
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-8 rounded-[14px] shadow-[6px_6px_54px_rgba(0,0,0,0.05)]">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h2 className="text-[24px] font-bold text-ink mb-1">{product.cropType}</h2>
                <span className="text-[14px] text-ink/50 uppercase tracking-wider font-semibold">{product.category || "General"}</span>
              </div>
              <div className="text-right">
                <div className="text-[28px] font-bold text-primary">ETB {product.price?.toLocaleString()}</div>
                <div className="text-[13px] text-ink/50">per {product.unit || "kg"}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
              <div className="bg-surface-muted p-4 rounded-[10px]">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[12px] text-ink/50 font-semibold uppercase">Quantity</span>
                </div>
                <div className="text-[18px] font-bold text-ink">{product.quantity} <span className="text-[14px] font-normal text-ink/50">{product.unit}</span></div>
              </div>
              <div className="bg-surface-muted p-4 rounded-[10px]">
                <div className="flex items-center gap-2 mb-2">
                  <svg className="w-4 h-4 text-warning" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" /></svg>
                  <span className="text-[12px] text-ink/50 font-semibold uppercase">Listed</span>
                </div>
                <div className="text-[14px] font-bold text-ink">{new Date(product.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</div>
              </div>
              <div className="bg-surface-muted p-4 rounded-[10px]">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[12px] text-ink/50 font-semibold uppercase">Region</span>
                </div>
                <div className="text-[14px] font-bold text-ink">{product.location?.region || product.farmerId?.location?.region || "N/A"}</div>
              </div>
              <div className="bg-surface-muted p-4 rounded-[10px]">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[12px] text-ink/50 font-semibold uppercase">Orders</span>
                </div>
                <div className="text-[18px] font-bold text-ink">{orders.length}</div>
              </div>
            </div>
            ...
            {product.description && (
              <div>
                ...
                <p className="text-[14px] text-ink/70 leading-relaxed bg-surface-muted p-5 rounded-[10px]">
                  {product.description}
                </p>
              </div>
            )}
          ...
          {/* Farmer Info Card */}
          <div className="bg-white p-8 rounded-[14px] shadow-[6px_6px_54px_rgba(0,0,0,0.05)]">
            <h3 className="text-[18px] font-bold text-ink mb-5 flex items-center gap-2">
            ...
            <div className="flex items-start gap-5">
              ...
              <div className="flex-1">
                <Link to={`/users/${product.farmerId?.customId || product.farmerId?._id}`} className="text-[18px] font-bold text-ink hover:text-primary hover:underline">
                  {product.farmerId?.name || "Unknown Farmer"}
                </Link>
                <div className="text-[13px] font-mono text-primary mb-3">{product.farmerId?.customId || ""}</div>
                <div className="flex flex-wrap gap-4 text-[14px] text-ink/60">
              ...
              <Link
                ...
                className="px-4 py-2 bg-surface-muted text-primary rounded-[10px] font-semibold text-[13px] hover:bg-primary hover:text-white transition-all flex-shrink-0"
              >
                View Profile
              </Link>
            </div>
      ...
      <div className="bg-white p-8 rounded-[14px] shadow-[6px_6px_54px_rgba(0,0,0,0.05)]">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-[20px] font-bold text-ink flex items-center gap-2">
            <svg className="w-5 h-5 text-warning" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V19.5a2.25 2.25 0 002.25 2.25h.75" /></svg>
            Recent Orders
            <span className="text-[14px] font-normal text-ink/40 ml-1">({orders.length})</span>
          </h3>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-soft">
                <th className="py-4 px-5 text-[14px] font-bold text-ink rounded-l-[12px]">Order ID</th>
                <th className="py-4 px-5 text-[14px] font-bold text-ink">Buyer</th>
              ...
              {orders.map(o => (
                <tr key={o._id} className="border-b border-[#979797]/15 last:border-0 hover:bg-[#F9FAFB] transition-colors">
                  <td className="py-4 px-5">
                    <Link to={`/orders/${o.customId || o._id}`} className="text-[13px] font-mono text-primary hover:underline">
                      {o.customId || `#${o._id.slice(-6)}`}
                    </Link>
                  </td>
                  <td className="py-4 px-5">
                    {o.buyerId ? (
                      <Link to={`/users/${o.buyerId.customId || o.buyerId._id}`} className="text-[14px] text-ink hover:text-primary hover:underline font-medium">
